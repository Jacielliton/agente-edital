...
import { 
  ...
  ArrowLeft, Scale, BarChart2, PieChart, BookOpenCheck, 
  SlidersHorizontal, X, GraduationCap, Wand2
} from "lucide-react";
    ...
    <div className="container">
      ...
      <header className="header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', margin: 0 }}>Constitucional, Penal, Administrativo e Processual focado em jurisprudência.</p>
        </div>
      ...
      {viewState === "topics" && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
          {direitoData.map(aula => (
            ...
            >
              <div style={{ fontSize: '0.8rem', fontWeight: 'bold', textTransform: 'uppercase', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                {aula.area}
              </div>
      ...
      {viewState === "lesson" && currentTopic && (
        <div className="panel" style={{ padding: '2.5rem' }}>
          ...
          <div style={{ marginBottom: '2rem', borderBottom: '2px solid var(--border)', paddingBottom: '1rem' }}>
            <span style={{ display: 'inline-block', padding: '4px 8px', background: 'var(--hover-bg)', color: 'var(--text-secondary)', fontSize: '0.85rem', borderRadius: '4px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '10px' }}>
              {currentTopic.area}
            </span>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: 'var(--heading-color)', fontSize: '1.3rem', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>📖 1. Base Doutrinária</h3>
            <div style={{ color: 'var(--text-main)', lineHeight: '1.6' }}><ReactMarkdown>{currentTopic.def}</ReactMarkdown></div>
          </div>

          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: 'var(--heading-color)', fontSize: '1.3rem', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>🔍 2. Análise Jurisprudencial</h3>
            <div style={{ color: 'var(--text-main)', lineHeight: '1.6' }}><ReactMarkdown>{currentTopic.deep}</ReactMarkdown></div>
          </div>

          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: 'var(--heading-color)', fontSize: '1.3rem', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>📝 3. Casos Concretos</h3>
          ...
          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: 'var(--heading-color)', fontSize: '1.3rem', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>🎯 4. Visão do Examinador (CESPE)</h3>
            <div style={{ background: 'var(--primary-light)', border: '1px solid var(--primary)', padding: '1.5rem', borderRadius: '8px', color: 'var(--text-main)' }}>
              <strong>Estratégia de Prova:</strong> <ReactMarkdown>{currentTopic.cespeTip}</ReactMarkdown>
            </div>
          </div>

          <div style={{ marginBottom: '2rem' }}>
            <h3 style={{ color: 'var(--heading-color)', fontSize: '1.3rem', marginBottom: '0.8rem', display: 'flex', alignItems: 'center', gap: '8px' }}>⚠️ 5. Pegadinhas Clássicas</h3>
            <div style={{ background: 'var(--error-bg)', border: '1px solid var(--error-border, #fecaca)', padding: '1.5rem', borderRadius: '8px', color: 'var(--error-text)' }}>
              <strong>Alerta Jurisprudencial:</strong> <ReactMarkdown>{currentTopic.trap}</ReactMarkdown>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1.5rem', marginTop: '2rem' }}>
             ...
             <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
                ...
                <div style={{ flex: '1 1 200px' }}>
                   <label className="label">Extensão da Prova</label>
                ...
                <div style={{ flex: '1 1 200px', display: 'flex', alignItems: 'center', cursor: 'pointer' }} onClick={() => setConfigExamMode(!configExamMode)}>
                   <div style={{ flex: 1 }}>
                     ...
                     <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Julgue tudo e corrija no final</span>
                   </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          {configExamMode && !isExamFinished && (
            <div style={{ background: 'var(--primary-light)', padding: '15px 20px', borderRadius: '12px', border: '1px solid var(--primary)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Respostas ocultadas. Entregue a prova para analisar.</span>
            </div>
          )}
          ...
          {currentQuestions.map((q, index) => {
            ...
            return (
              <React.Fragment key={q.id}>
                {/* NOVO BLOCO: Renderiza a Situação Hipotética se ela existir para esta questão */}
                {q.textoBase && q.textoBase.trim() !== "" && (
                  <div style={{ background: 'var(--primary-light)', border: '1px solid var(--border)', borderLeft: '5px solid var(--primary)', padding: '20px', borderRadius: '8px', marginBottom: '15px', color: 'var(--text-main)', fontSize: '1.05rem', lineHeight: '1.6', boxShadow: 'var(--shadow-sm)' }}>
                    <div style={{ fontWeight: 'bold', color: 'var(--primary)', marginBottom: '10px', fontSize: '0.9rem', textTransform: 'uppercase' }}>
                      📖 Situação Hipotética / Texto Base:
                    </div>
                    <ReactMarkdown>{q.textoBase}</ReactMarkdown>
                  </div>
                )}
                      ...
                      {showExp && (
                        <div className="quiz-explanation" style={{ marginTop: '20px', animation: 'fadeIn 0.3s ease-out' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 'bold', marginBottom: '8px', color: isCorrect ? 'var(--success-text)' : 'var(--error-text)' }}>
                            ...
                            {isCorrect ? "Julgamento Perfeito!" : "Erro na Análise Fática."} (Gabarito: {q.gabarito})
                          </div>
          ...
          {showLessonAction && (
            <div style={{ background: 'var(--warning-bg, #fffbeb)', border: '1px solid #fcd34d', padding: '25px', borderRadius: '12px', textAlign: 'center', margin: '20px auto', maxWidth: '500px', boxShadow: 'var(--shadow-md)' }}>
              <h4 style={{ margin: '0 0 10px 0', color: '#b45309', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontSize: '1.2rem' }}>
                <GraduationCap size={24} /> Desembargador de IA
              </h4>
              <p style={{ color: '#92400e', fontSize: '0.95rem', marginBottom: '20px' }}>
                Houve {wrongCount} erro(s) na subsunção dos fatos à norma. Gerar um resumo jurisprudencial das regras violadas?
              </p>
              <button onClick={() => generateLesson(getWrongQuestions())} className="btn" style={{ background: '#f59e0b', color: 'white', border: 'none', width: '100%', padding: '12px', fontSize: '1.05rem', fontWeight: 'bold', display: 'flex', justifyContent: 'center', gap: '8px' }}>
                <Wand2 size={20} /> Analisar Fundamentação
              </button>
            </div>
          )}
        </div>
          ...
          <div className="panel" style={{ width: '100%', maxWidth: '800px', maxHeight: '90vh', display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden', animation: 'fadeIn 0.2s ease-out', border: '2px solid #f59e0b' }}>
            <div style={{ background: '#f59e0b', color: 'white', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <h2 style={{ margin: 0, fontSize: '1.25rem', display: 'flex', alignItems: 'center', gap: '10px' }}>
                <GraduationCap size={24} /> Parecer de Revisão
              </h2>
            ...
            {providerTab === "openrouter" && (
              <div style={{ marginBottom: '20px' }}>
                <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '15px' }}>
                  Conecte sua conta OpenRouter para acesso a dezenas de modelos de IA.
                </p>
            ...
            {providerTab === "aistudio" && (
              <div style={{ marginBottom: '20px' }}>
                <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '15px', lineHeight: '1.4' }}>
                  O Google AI Studio exige a geração manual da chave de acesso utilizando o seu Gmail.
                </p>
                <a 
          ...
          <div className="panel" style={{ width: '100%', maxWidth: '500px', padding: 0, overflow: 'hidden', animation: 'fadeIn 0.2s ease-out' }}>
            <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--bg)' }}>
              <h2 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--heading-color)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <PieChart size={22} color="var(--primary)" /> Meu Desempenho Jurídico
              </h2>
            ...
            <div style={{ padding: '20px' }}>
              <div style={{ display: 'flex', gap: '15px', marginBottom: '25px' }}>
                <div style={{ flex: 1, background: 'var(--bg)', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Julgadas</span>
                </div>
                <div style={{ flex: 1, background: 'var(--success-bg)', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Acertos</span>
                </div>
                <div style={{ flex: 1, background: 'var(--error-bg)', padding: '15px', borderRadius: '10px', textAlign: 'center' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Erros</span>
                </div>
              </div>

              <h3 style={{ fontSize: '1rem', color: 'var(--heading-color)', marginBottom: '15px' }}>Desempenho por Instituto</h3>
              ...
              <button onClick={() => {
                if(window.confirm("Apagar todo o histórico de Direito?")) {
